...
| 1292 race-safe tests across 23 tested packages | `go test -race -count=1 ./...` — 1292 passing tests across 23 packages with test files; 4 packages have no test files (`cmd/shardforge-bench`, `cmd/shardforge-dashboard`, `cmd/shardforge-node`, `internal/storage`) |
...
| Durable replication restart recovery demo | `scripts/repl_restart_demo_{up,smoke,down}.sh` — 16 checks: primary journal survives restart, follower cursor restored after restart, replication resumes from correct position, idempotent pull; `make repl-restart-demo-{up,smoke,down}` |
...
| Operator-controlled manual promotion (planned failover, no automatic failover) | `POST /replication/quiesce` on primary: write-fence via RWMutex write gate, persist quiesce record (NewQuiesceID crypto/rand, CRC32 checksum, quiesceMu-serialized). `POST /replication/promote` on follower: 16 precondition checks (cursor re-validated UNDER WLock post-lock), promotionBarrier+double-check drain, CreateJournalBaseline (idempotent), SavePromotionRecord (commit point), DurableLog starts at N+1. `ApplyReplicationEntries` split into public guarded wrapper + private locked helper; `SyncFromPrimary` calls locked path directly. All 3 barrier returns wrap `ErrPromotionInProgress` (HTTP 409 from sync handler). Cross-validated startup: baseline.base_seq == promotion_record.inherited_last_seq. Startup fails on replication-state store errors. Quiesce retry removes intent on success. 98 Phase 28 net-new tests (`phase28_hardening_test.go`: 31, `phase28_safety_test.go`: 28, `journal_baseline_test.go`: 22, `quiesce_store_test.go`: 17). See `docs/MANUAL_PROMOTION_DESIGN.md`. NOT automatic failover; requires operator to stop old primary first. |
...
ops simulation tools — 1292 race-safe tests across 28 phases, reproducible benchmarks. All 28 approved phases complete and locked. Phase 29 has not started.
```
