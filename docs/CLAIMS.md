...
| 1096 race-safe tests across 23 packages | `go test -race -count=1 ./...` — 1096 passing tests, 23 packages with test files, 4 packages with no test files (`cmd/shardforge-bench`, `cmd/shardforge-dashboard`, `cmd/shardforge-node`, `internal/storage`) |
...
| 1096 race-safe tests across 23 packages | `go test -race -count=1 ./...` — 1096 passing tests |
