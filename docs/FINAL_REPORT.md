...
All 27 phases complete. `go test -race -count=1 ./...` → 1106 tests pass across 23 packages. Smoke demos: cluster (25/25), repl (16/16), repl-restart (16/16), repl-auto (24/24).
...
- Worker `lifecycleMu`: `start()` holds `lifecycleMu` across cancel-assignment + `wg.Add(1)` + goroutine launch; `stop()` acquires and releases `lifecycleMu` before reading cancel — guarantees `wg.Add` happens-before `wg.Wait` even if `stop()` races `start()` concurrently. Proven by `TestBackgroundSync_Worker_ConcurrentStartStop` (100-iteration race loop).
...
- Server lifecycle tests (9 new): double-StartBackground, StartBackground after Close, concurrent StartBackground+Close, follower bg worker started exactly once; in `server_lifecycle_test.go`
...
- Phase 27 net-new test accounting: 47 (`background_sync_test.go`) + 26 (`replication_phase27_test.go`) + 9 (`server_lifecycle_test.go`) + 1 (`replicator_test.go#PrimaryLatestSeq_Reported`) = 83 (not 72)
