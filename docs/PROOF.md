...
## Summary Table (Phase 28 — Manual Promotion and Controlled Failover, Hardening Pass 4 — merged and validation-locked 2026-06-18)
...
| 28 | `internal/replnet/quiesce_store.go` (NewQuiesceID→error, NewQuiesceRecord→error, quiesceIDFn seam), `internal/replnet/journal_baseline.go` (CreateJournalBaseline idempotent, JournalBaselineExists, ErrJournalBaselineConflict, ErrJournalBaselineMaxUint64, journalCompatibilityCheck stat-first+error propagation), `internal/node/server.go` (quiesceMu, promoteMu, promotionBarrier, quiesce_failed_fenced, pendingQuiesceRecord, runtimeSnapshot/runtimeState(), cross-validation in resolveRuntimeRole, double-check barrier in SyncFromPrimary, applyReplicationEntriesLocked split, restartBackgroundWorkerAfterPromotionFailure idempotent, ErrPromotionInProgress wrapping, bgEnabled race fix, BackgroundSyncStatus under mutex, SyncFromPrimary replicator capture under mutex, removeQuiesceIntentFn seam, ApplyReplicationEntries follower-only), `internal/node/handlers.go` (writeJSONError, explain fencing, handleQuiesce/handlePromote rewrite, pre-commit revert, validatePromotionPreconditionsLocked under WLock, handleReplicationSync 409 for promotion, quiesce intent truthful cleanup, cleanup_pending vs active intent state), `internal/node/types.go` (Phase 28 ReplicationStatusResponse fields), `internal/node/phase28_hardening_test.go` (31 tests), `internal/node/phase28_safety_test.go` (28 tests), `internal/replnet/journal_baseline_test.go` (22 tests, all Phase 28 new), `internal/replnet/quiesce_store_test.go` (17 tests, all Phase 28 new), `internal/node/phase28_test_helpers_test.go`, `docs/MANUAL_PROMOTION_DESIGN.md` | COMPLETE | 98 new (1292 total) | — | quiesceMu serializes all concurrent quiesce; quiesce_failed_fenced with pendingQuiesceRecord retry; entropy failure aborts before gate closes; s.Addr() (not :0); handleExplainPut/Delete fenced; runtimeSnapshot atomic snapshot; promoteMu serializes promotion; promotionBarrier+double-check drain; CreateJournalBaseline idempotent (phase-1 commit); cross-validation: new_role=="primary", non-empty fields, MaxUint64 check, baseline exists, BaseSeq==InheritedLastSeq; pre-commit revert, post-commit preserve; writeJSONError stable code field; typed ReplicationStatusResponse Phase 28 fields; orphan baseline safe on startup. Pass 3: cursor re-validated UNDER WLock; ApplyReplicationEntries split; startup fails on state-store errors; journalCompatibilityCheck stat-first; atomic bgWorker restart; ErrPromotionInProgress from all 3 barriers (HTTP 409); quiesce retry removes intent; bgEnabled race fix. Pass 4: idempotent bgWorker restart (never replaces live worker); BackgroundSyncStatus under mutex; SyncFromPrimary replicator capture under mutex; ApplyReplicationEntries follower-only (ErrNotFollower for primary/standalone/promoted-primary); removeQuiesceIntentFn seam; quiesceIntentActive truthful (cleared only on successful removal); cleanup_pending vs active intent state; deterministic Close-vs-restart race test (WaitGroup barrier, 50 iterations). NOT automatic failover; operator must stop old primary. |
...
**Current test pass status:** 1292 tests pass across 23 packages (race detector on) on Apple M3 darwin/arm64, Go 1.26. All 28 approved phases complete and locked. Phase 28 squash-merged to main at `b6965e839baf1ddaeeb6e64a69c781775d5e1396` (accepted PR head `4eb28c52795ae0cc6d98f4a29fb5909166b0e547`, 2026-06-18). Phase 29 has not started.

```
go test -race -count=1 -v ./... | grep -c "^--- PASS:" → 1292
```
...
### Test breakdown (61 Phase 26 tests, 1023 total)
...
```
go test -race -count=1 ./... → 1023 tests, 0 failures
