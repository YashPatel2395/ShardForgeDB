...
## Summary Table (Phase 28 — Manual Promotion and Controlled Failover, Hardening Pass)
...
| 28 | `internal/replnet/quiesce_store.go` (NewQuiesceID→error, NewQuiesceRecord→error, quiesceIDFn seam), `internal/replnet/journal_baseline.go` (CreateJournalBaseline idempotent, JournalBaselineExists, ErrJournalBaselineConflict, ErrJournalBaselineMaxUint64, journalCompatibilityCheck stat-first+error propagation), `internal/node/server.go` (quiesceMu, promoteMu, promotionBarrier, quiesce_failed_fenced, pendingQuiesceRecord, runtimeSnapshot/runtimeState(), cross-validation in resolveRuntimeRole, double-check barrier in SyncFromPrimary, applyReplicationEntriesLocked split, restartBackgroundWorkerAfterPromotionFailure, ErrPromotionInProgress wrapping, bgEnabled race fix), `internal/node/handlers.go` (writeJSONError, explain fencing, handleQuiesce/handlePromote rewrite, pre-commit revert, validatePromotionPreconditionsLocked under WLock, handleReplicationSync 409 for promotion, quiesce retry intent cleanup), `internal/node/types.go` (Phase 28 ReplicationStatusResponse fields), `internal/node/phase28_hardening_test.go` (26 tests), `internal/node/phase28_safety_test.go` (18 tests), `internal/node/phase28_test_helpers_test.go`, `docs/MANUAL_PROMOTION_DESIGN.md` | COMPLETE | 165 new (1282 total) | — | quiesceMu serializes all concurrent quiesce; quiesce_failed_fenced with pendingQuiesceRecord retry; entropy failure aborts before gate closes; s.Addr() (not :0); handleExplainPut/Delete fenced; runtimeSnapshot atomic snapshot; promoteMu serializes promotion; promotionBarrier+double-check drain; CreateJournalBaseline idempotent (phase-1 commit); cross-validation: new_role=="primary", non-empty fields, MaxUint64 check, baseline exists, BaseSeq==InheritedLastSeq; pre-commit revert, post-commit preserve; writeJSONError stable code field; typed ReplicationStatusResponse Phase 28 fields; orphan baseline safe on startup. Hardening pass 3: cursor re-validated UNDER WLock; ApplyReplicationEntries split; startup fails on state-store errors; journalCompatibilityCheck stat-first; atomic bgWorker restart; ErrPromotionInProgress from all 3 barriers (HTTP 409); quiesce retry removes intent; bgEnabled race fix. NOT automatic failover; operator must stop old primary. |
...
**Current test pass status:** 1282 tests pass across 23 packages (race detector on) on Apple M3 darwin/arm64, Go 1.26.

```
go test -race -count=1 -v ./... | grep -c "^--- PASS:" → 1282
```
