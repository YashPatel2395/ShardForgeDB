...
| 28 | `internal/replnet/quiesce_store.go` (NewQuiesceID→error, NewQuiesceRecord→error, quiesceIDFn seam), `internal/replnet/journal_baseline.go` (CreateJournalBaseline idempotent, JournalBaselineExists, ErrJournalBaselineConflict, ErrJournalBaselineMaxUint64), `internal/node/server.go` (quiesceMu, promoteMu, promotionBarrier, quiesce_failed_fenced, pendingQuiesceRecord, runtimeSnapshot/runtimeState(), cross-validation in resolveRuntimeRole, double-check barrier in SyncFromPrimary), `internal/node/handlers.go` (writeJSONError, explain fencing, handleQuiesce/handlePromote rewrite, pre-commit revert), `internal/node/types.go` (Phase 28 ReplicationStatusResponse fields), `internal/node/phase28_hardening_test.go` (26 tests), `internal/node/phase28_test_helpers_test.go`, `docs/MANUAL_PROMOTION_DESIGN.md` | COMPLETE | 147 new (1259 total) | — | quiesceMu serializes all concurrent quiesce; quiesce_failed_fenced with pendingQuiesceRecord retry; entropy failure aborts before gate closes; s.Addr() (not :0); handleExplainPut/Delete fenced; runtimeSnapshot atomic snapshot; promoteMu serializes promotion; promotionBarrier+double-check drain; CreateJournalBaseline idempotent (phase-1 commit); cross-validation: new_role=="primary", non-empty fields, MaxUint64 check, baseline exists, BaseSeq==InheritedLastSeq; pre-commit revert, post-commit preserve; writeJSONError stable code field; typed ReplicationStatusResponse Phase 28 fields; orphan baseline safe on startup. NOT automatic failover; operator must stop old primary. |
...
**Current test pass status:** 1259 tests pass across 23 packages (race detector on) on Apple M3 darwin/arm64, Go 1.26.

```
go test -race -count=1 -v ./... | grep -c "^--- PASS:" → 1259
```
